...
(define-public (borrow (update (buff 8192)) (amount uint) (maybe-user (optional principal)))
  (begin
    ...
    (try! (accrue-interest))
    (asserts! (>= (contract-call? 'SP3M2BYF7RGF8WKW5FVDNJ6WR8D7AR9BHDXAKPXZE.state-v1 get-borrowable-balance) amount) ERR-INSUFFICIENT-FREE-LIQUIDITY)
    (let
      (
        ...
        (current-debt (contract-call? 'SP3M2BYF7RGF8WKW5FVDNJ6WR8D7AR9BHDXAKPXZE.math-v1 convert-to-debt-assets debt-params current-debt-shares true))
        (new-debt-shares (contract-call? 'SP3M2BYF7RGF8WKW5FVDNJ6WR8D7AR9BHDXAKPXZE.math-v1 convert-to-debt-shares debt-params amount true))
        ...
        (new-current-debt-adjusted (contract-call? 'SP3M2BYF7RGF8WKW5FVDNJ6WR8D7AR9BHDXAKPXZE.math-v1 get-market-asset-value market-asset-price new-current-debt))
      )
      (asserts! (<= new-current-debt-adjusted total-max-ltv) ERR-MAX-LTV)
      (try! (contract-call? 'SP3M2BYF7RGF8WKW5FVDNJ6WR8D7AR9BHDXAKPXZE.state-v1 update-borrow-state {
        user: user,
  ...
  (begin
    ...
    (let
      (
        ...
        (interest-params (contract-call? 'SP3M2BYF7RGF8WKW5FVDNJ6WR8D7AR9BHDXAKPXZE.state-v1 get-open-interest))
        ...
        (lp-open-interest-without-principal (contract-call? 'SP3M2BYF7RGF8WKW5FVDNJ6WR8D7AR9BHDXAKPXZE.math-v1 safe-sub (get lp-open-interest interest-params) total-borrowed-amount))
        ...
        (protocol-part (contract-call? 'SP3M2BYF7RGF8WKW5FVDNJ6WR8D7AR9BHDXAKPXZE.math-v1 safe-div (* interest-part (get protocol-open-interest interest-params)) open-interest-without-principal))
        (staked-part (contract-call? 'SP3M2BYF7RGF8WKW5FVDNJ6WR8D7AR9BHDXAKPXZE.math-v1 safe-div (* interest-part (get staked-open-interest interest-params)) open-interest-without-principal))
        ...
        (total-user-debt-shares (unwrap! (contract-call? 'SP3M2BYF7RGF8WKW5FVDNJ6WR8D7AR9BHDXAKPXZE.math-v1 sub (get debt-shares position) shares) ERR-NOT-ENOUGH-SHARES))
      ...
      (try! (contract-call? 'SP3M2BYF7RGF8WKW5FVDNJ6WR8D7AR9BHDXAKPXZE.state-v1 update-repay-state {
        user: user,
        user-debt-shares: total-user-debt-shares,
  ...
  (begin
    (let
      (
        ...
        (add-collateral-params (try! (contract-call? 'SP3M2BYF7RGF8WKW5FVDNJ6WR8D7AR9BHDXAKPXZE.state-v1 get-collateral-params collateral-token user)))
      ...
      (try! (contract-call? 'SP3M2BYF7RGF8WKW5FVDNJ6WR8D7AR9BHDXAKPXZE.state-v1 update-add-collateral collateral {
        amount: amount,
  ...
  (begin
    ...
    (let
      (
        ...
        (verified (try! (contract-call? .pyth-adapter-v1 verify-update update)))
        (collateral-token (contract-of collateral))
        (remove-collateral-params (try! (contract-call? 'SP3M2BYF7RGF8WKW5FVDNJ6WR8D7AR9BHDXAKPXZE.state-v1 get-collateral-params collateral-token user)))
      ...
      (try! (contract-call? 'SP3M2BYF7RGF8WKW5FVDNJ6WR8D7AR9BHDXAKPXZE.state-v1 transfer-to collateral user amount))
      (print {
          collateral: collateral-token,
...
(define-read-only (get-user-collaterals-value (account principal) (collateral-prices (list 10 uint)))
  (match (contract-call? 'SP3M2BYF7RGF8WKW5FVDNJ6WR8D7AR9BHDXAKPXZE.state-v1 get-user-position account)
  position
...
(define-private (remove-user-collateral (user principal) (prev-amount uint) (amount uint) (collateral principal) (debt-shares uint) (position-collaterals (list 10 principal)) (borrowed-amount uint) (borrowed-block uint))
  (let ((remaining-amount (unwrap! (contract-call? 'SP3M2BYF7RGF8WKW5FVDNJ6WR8D7AR9BHDXAKPXZE.math-v1 sub prev-amount amount) ERR-INSUFFICIENT-BALANCE)))
    (if (is-eq remaining-amount u0)
      (let ((updated-position-collaterals (contract-call? 'SP3M2BYF7RGF8WKW5FVDNJ6WR8D7AR9BHDXAKPXZE.state-v1 remove-item position-collaterals collateral)))
        ;; remove the collateral since there is no user collateral left
        (try! (contract-call? 'SP3M2BYF7RGF8WKW5FVDNJ6WR8D7AR9BHDXAKPXZE.state-v1 update-remove-collateral user collateral debt-shares (get new-list updated-position-collaterals) borrowed-amount borrowed-block))
        (ok {
      ...
      (begin
        ...
        (try! (contract-call? 'SP3M2BYF7RGF8WKW5FVDNJ6WR8D7AR9BHDXAKPXZE.state-v1 update-user-collateral user collateral remaining-amount))
        (ok {
          remaining-amount: remaining-amount,
...
(define-private (max-repay-amount (amount uint) (total-user-debt-shares uint))
  (let (
      (debt-params (contract-call? 'SP3M2BYF7RGF8WKW5FVDNJ6WR8D7AR9BHDXAKPXZE.state-v1 get-debt-params))
      (current-debt (contract-call? 'SP3M2BYF7RGF8WKW5FVDNJ6WR8D7AR9BHDXAKPXZE.math-v1 convert-to-debt-assets debt-params total-user-debt-shares true))
      (repay-amount (if (>= amount current-debt) current-debt amount))
      (shares (if (is-eq repay-amount current-debt) total-user-debt-shares (contract-call? 'SP3M2BYF7RGF8WKW5FVDNJ6WR8D7AR9BHDXAKPXZE.math-v1 convert-to-debt-shares debt-params amount false)))
    )
    {
