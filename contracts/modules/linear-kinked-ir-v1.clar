...
(define-data-var ir-slope-1 uint u30000000000)
(define-data-var ir-slope-2 uint u3100000000000)
(define-data-var utilization-kink uint u500000000000)
(define-data-var base-ir uint u4000000000) ;; interest when utilization is 0
...
(define-read-only (accrue-interest (last-accrued-block-time uint) (lp-open-interest uint) (staked-open-interest uint) (staking-reward-percentage uint) (protocol-open-interest uint) (protocol-reserve-percentage uint) (total-assets uint))
  (let (
    ...
    (premature-return (asserts!
      (not (or (is-eq u0 elapsed-block-time) (not (contract-call? 'SP35E2BBMDT2Y1HB0NTK139YBGYV3PAPK3WA8BRNA.state-v1 is-interest-accrual-enabled))))
      (ok {
...
(define-read-only (calc-total-interest (ir uint) (open-ir uint))
  (let ((interest-factor (- ir one-12))
    (total-interest (contract-call? 'SP35E2BBMDT2Y1HB0NTK139YBGYV3PAPK3WA8BRNA.math-v1 divide-round-up (* open-ir interest-factor) one-12)))
    total-interest
))
