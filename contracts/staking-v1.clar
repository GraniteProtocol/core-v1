...
(define-public (update-withdrawal-finalization-period (new-value uint))
  (begin
    (asserts! (is-eq contract-caller (contract-call? 'SP3M2BYF7RGF8WKW5FVDNJ6WR8D7AR9BHDXAKPXZE.state-v1 get-governance)) ERR-NOT-GOVERNANCE)
    (print {
...
(define-public (transfer (amount uint) (sender principal) (recipient principal) (memo (optional (buff 34))))
  (begin
    ...
    (asserts! (contract-call? 'SP3M2BYF7RGF8WKW5FVDNJ6WR8D7AR9BHDXAKPXZE.state-v1 is-staking-enabled) ERR-STAKING-DISABLED)
...
(define-read-only (get-decimals)
  ...
  (contract-call? 'SP3M2BYF7RGF8WKW5FVDNJ6WR8D7AR9BHDXAKPXZE.state-v1 get-decimals)
)
...
(define-read-only (convert-to-staked-lp-tokens (lp-tokens uint) (round-up bool))
  (let (
    ...
    (if (is-eq total-staked-lp-tokens u0)
      lp-tokens
      (contract-call? 'SP3M2BYF7RGF8WKW5FVDNJ6WR8D7AR9BHDXAKPXZE.math-v1 divide round-up (* total-staked-lp-tokens lp-tokens) total-lp-tokens)
    )
))
...
(define-read-only (convert-to-lp-tokens (staked-lp-tokens uint) (round-up bool))
  (let (
    ...
    (if (is-eq total-lp-tokens u0)
      staked-lp-tokens
      (contract-call? 'SP3M2BYF7RGF8WKW5FVDNJ6WR8D7AR9BHDXAKPXZE.math-v1 divide round-up (* staked-lp-tokens total-lp-tokens) total-staked-lp-tokens)
    )
))
...
(define-read-only (convert-to-withdrawal-shares (lp-tokens uint))
  (let (
    ...
    (if (is-eq total-withdrawal-shares u0)
      lp-tokens
      (contract-call? 'SP3M2BYF7RGF8WKW5FVDNJ6WR8D7AR9BHDXAKPXZE.math-v1 divide false (* total-withdrawal-shares lp-tokens) total-withdrawal-lp-tokens)
    )
))
...
(define-read-only (convert-to-withdrawal-lp-tokens (withdrawal-shares uint))
  (let (
    ...
    (if (is-eq total-withdrawal-lp-tokens u0)
      u0
      (contract-call? 'SP3M2BYF7RGF8WKW5FVDNJ6WR8D7AR9BHDXAKPXZE.math-v1 divide false (* withdrawal-shares total-withdrawal-lp-tokens) total-withdrawal-shares)
    )
))
...
(define-public (stake (lp-tokens uint))
  (begin
    ...
    (let (
      ...
      (try! (contract-call? 'SP3M2BYF7RGF8WKW5FVDNJ6WR8D7AR9BHDXAKPXZE.state-v1 transfer lp-tokens contract-caller (as-contract contract-caller) none))
...
(define-public (increase-lp-staked-balance (lp-tokens uint))
  (begin
    (try! (contract-call? 'SP3M2BYF7RGF8WKW5FVDNJ6WR8D7AR9BHDXAKPXZE.state-v1 is-allowed-contract contract-caller))
...
(define-public (reconcile-lp-token-balance)
  (let (
      (current-balance (unwrap! (contract-call? 'SP3M2BYF7RGF8WKW5FVDNJ6WR8D7AR9BHDXAKPXZE.state-v1 get-balance (as-contract contract-caller)) ERR-LP-TOKEN-SUPPLY))
    ...
    (try! (check-staking-enabled))
    (try! (accrue-interest))
    (asserts! (is-eq (contract-call? 'SP3M2BYF7RGF8WKW5FVDNJ6WR8D7AR9BHDXAKPXZE.state-v1 get-governance) contract-caller) ERR-NOT-GOVERNANCE)
...
(define-public (finalize-unstake (index uint))
  (let (
    ...
    (try! (if (> lp-tokens-to-return u0)
      (as-contract (contract-call? 'SP3M2BYF7RGF8WKW5FVDNJ6WR8D7AR9BHDXAKPXZE.state-v1 transfer lp-tokens-to-return (as-contract contract-caller) user none))
      SUCCESS
    ))
    (if (not (var-get staking-wiped-out))
      ...
      (var-set unfinalized-withdrawals {
        lp-tokens: (contract-call? 'SP3M2BYF7RGF8WKW5FVDNJ6WR8D7AR9BHDXAKPXZE.math-v1 safe-sub (get lp-tokens unfinalized-withdrawal-info) lp-tokens-to-return),
        shares: (contract-call? 'SP3M2BYF7RGF8WKW5FVDNJ6WR8D7AR9BHDXAKPXZE.math-v1 safe-sub (get shares unfinalized-withdrawal-info) withdraw-shares),
      })
    )
...
(define-private (accrue-interest)
  (let (
    ...
    (accrued-interest (try! (contract-call? .linear-kinked-ir-v1 accrue-interest
      ...
      (try! (contract-call? 'SP3M2BYF7RGF8WKW5FVDNJ6WR8D7AR9BHDXAKPXZE.staking-reward-v1 calculate-staking-reward-percentage (get-active-staked-lp-tokens)))
...
(define-private (check-staking-enabled)
  (let (
    (staking-disabled (not (contract-call? 'SP3M2BYF7RGF8WKW5FVDNJ6WR8D7AR9BHDXAKPXZE.state-v1 is-staking-enabled)))
