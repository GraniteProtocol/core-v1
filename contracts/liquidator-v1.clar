...
(define-private (accrue-interest)
  (let (
    ...
    (accrued-interest (try! (contract-call? .linear-kinked-ir-v1 accrue-interest
      ...
      (try! (contract-call? 'SP3Y6GFKWN50HPA8RKRXMY0EXAJR9VXPY899P88JN.staking-reward-v1 calculate-staking-reward-percentage (contract-call? .staking-v1 get-active-staked-lp-tokens)))
    ...
    (let (
      ...
      (if (or (> total-collateral-value u0) (is-eq remaining-debt u0))
        ...
        (let (
            ...
            (is-wiped (contract-call? .staking-v1 is-staking-wiped-out))
            (effective-staked-lp-tokens (if is-wiped u0 staked-lp-tokens))
            (burned-staking-lp-tokens (try! (contract-call? 'SP3Y6GFKWN50HPA8RKRXMY0EXAJR9VXPY899P88JN.state-v1 socialize-user-bad-debt user remaining-debt lp-part staked-part protocol-part updated-total-borrowed-amount .staking-v1 effective-staked-lp-tokens)))
          )
          (print {
            user: user,
