...
;; What a liquidation could still seize from this collateral at a full-value repay.
;; Zero for a residue the seizure math can never extract: the conversion floors to
;; zero token units and state-v1.transfer-to rejects a zero-amount transfer.
(define-private (max-seizable-for-collateral (collateral principal) (user principal) (collateral-price uint))
  (let (
      ...
      (collateral-decimals (default-to u8 (get decimals collateral-info)))
      (liquidation-premium (default-to u0 (get liquidation-premium collateral-info)))
      (collateral-value (get-collateral-value collateral user collateral-price))
      (repay-without-discount (contract-call? .math-v1 divide-round-up (* collateral-value SCALING-FACTOR) (+ liquidation-premium SCALING-FACTOR)))
    )
    (match (calc-collateral-to-give repay-without-discount liquidation-premium collateral-price collateral-decimals deposited)
      seizable seizable
      e u0)
))
