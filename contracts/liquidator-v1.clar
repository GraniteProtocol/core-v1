...
(define-private (execute-liquidation (verified { timestamp: uint, feeds: (list 16 { feed-id: uint, price: int, exponent: int, confidence: (optional uint) }) }) (user principal) (collateral <token-trait>) (liquidator-repay-amount uint) (min-collateral-expected uint))
  (let
      (
        (collateral-token (contract-of collateral))
        (user-position-data (contract-call? 'SP35E2BBMDT2Y1HB0NTK139YBGYV3PAPK3WA8BRNA.state-v1 get-borrow-repay-params user))
        ...
        (repay-amount-raw (contract-call? 'SP35E2BBMDT2Y1HB0NTK139YBGYV3PAPK3WA8BRNA.math-v1 divide-round-up (* repay-amount-value PRICE-SCALING-FACTOR) market-asset-price))
        ...
        (interest-portion (contract-call? 'SP35E2BBMDT2Y1HB0NTK139YBGYV3PAPK3WA8BRNA.math-v1 calculate-interest-portions current-debt effective-borrowed-amount repay-amount))
        ...
        (open-interest-without-principal (contract-call? 'SP35E2BBMDT2Y1HB0NTK139YBGYV3PAPK3WA8BRNA.math-v1 safe-sub open-interest total-borrowed-amount))
        (lp-open-interest-without-principal (contract-call? 'SP35E2BBMDT2Y1HB0NTK139YBGYV3PAPK3WA8BRNA.math-v1 safe-sub (get lp-open-interest open-interest-info) total-borrowed-amount))
        ...
        (remaining-user-debt-shares (contract-call? 'SP35E2BBMDT2Y1HB0NTK139YBGYV3PAPK3WA8BRNA.math-v1 safe-sub (get debt-shares position-for-block-check) paid-shares))
        (updated-borrowed-amount (contract-call? 'SP35E2BBMDT2Y1HB0NTK139YBGYV3PAPK3WA8BRNA.math-v1 safe-sub effective-borrowed-amount principal-part))
        (updated-total-borrowed-amount (contract-call? 'SP35E2BBMDT2Y1HB0NTK139YBGYV3PAPK3WA8BRNA.math-v1 safe-sub total-borrowed-amount
      ...
      (let (
          ...
          (post-liq-collaterals (get collaterals (unwrap! (get user-position (contract-call? 'SP35E2BBMDT2Y1HB0NTK139YBGYV3PAPK3WA8BRNA.state-v1 get-borrow-repay-params user)) ERR-NO-POSITION)))
      ...
      (if (or (> total-collateral-value u0) (is-eq remaining-debt u0))
        ...
        (let (
            ...
            (interest-portion (if (>= remaining-debt user-borrowed-amount)
              (contract-call? 'SP35E2BBMDT2Y1HB0NTK139YBGYV3PAPK3WA8BRNA.math-v1 calculate-interest-portions remaining-debt user-borrowed-amount remaining-debt)
              {principal-part: remaining-debt, interest-part: u0}
            ))
            (principal-part (get principal-part interest-portion))
            (interest-part (get interest-part interest-portion))
            (open-interest-without-principal (contract-call? 'SP35E2BBMDT2Y1HB0NTK139YBGYV3PAPK3WA8BRNA.math-v1 safe-sub total-open-interest total-borrowed-amount))
            (lp-open-interest-without-principal (contract-call? 'SP35E2BBMDT2Y1HB0NTK139YBGYV3PAPK3WA8BRNA.math-v1 safe-sub lp-open-interest-val total-borrowed-amount))
            (lp-interest-part (contract-call? 'SP35E2BBMDT2Y1HB0NTK139YBGYV3PAPK3WA8BRNA.math-v1 safe-div (* interest-part lp-open-interest-without-principal) open-interest-without-principal))
            ...
            (updated-total-borrowed-amount (contract-call? 'SP35E2BBMDT2Y1HB0NTK139YBGYV3PAPK3WA8BRNA.math-v1 safe-sub total-borrowed-amount user-borrowed-amount))
            (staked-lp-tokens (contract-call? .staking-v1 get-total-staked-lp-tokens))
            (is-wiped (contract-call? .staking-v1 is-staking-wiped-out))
            (effective-staked-lp-tokens (if is-wiped u0 staked-lp-tokens))
            (burned-staking-lp-tokens (try! (contract-call? 'SP35E2BBMDT2Y1HB0NTK139YBGYV3PAPK3WA8BRNA.state-v1 socialize-user-bad-debt user remaining-debt lp-part staked-part protocol-part updated-total-borrowed-amount .staking-v1 effective-staked-lp-tokens)))
          )
          (print {
            user: user,
